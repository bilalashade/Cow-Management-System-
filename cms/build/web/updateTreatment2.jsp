    ...
    <body>
        <%
            ...
            try {
                ...
                String queryh = "UPDATE treatment SET medicineID = '" + medicineID + "', medicineMethod = '" + medicineMethod + "'"
                        + "WHERE checkupDateTime='" + checkupDateTime + "' && treatmentDateTime='" + treatmentDateTime + "'";
                int h = statement.executeUpdate(queryh);

                if (h == 1) {
                    out.println("<script>alert('Treatment Information Has Been Successfully Updated');document.location.href='managePastTreatment.jsp';</script>");
                } else {
                    out.println("<script>alert('Treatment Information Failed To Be Updated');window.history.back();</script>");
                }
